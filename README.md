# ether-proxy / Updated for 2020 and beyond...

Ethereum mining proxy with web-interface.

# Run with

```chmod +x run.sh ; docker run --restart unless-stopped -p 8546:8546 -p 8080:8080 -v $(pwd)/run.sh:/run.sh -v $(pwd)/config.json:/config.json mics8128/ether-proxy-docker```
